...
const fieldArrayName = 'secondaryContact';

export const SecondaryContact = ({ setForm }: { setForm: CallableFunction }) => {
  ...
  const saveData = (data: any) => {
    ...
    setState({ ...state, ...data });
    setForm({ form: nextOverride != '' ? nextOverride : 'emergencyContacts' });
  };
    ...
    <Form onSubmit={handleSubmit(saveData)}>
      <fieldset>
        <div className="form-section">
          <div className="form-subsection shadow">
            ...
            <div className="form-row">
              ...
              <Field label="State" error={errors?.[fieldArrayName]?.state} className="w-1/3">
                <select {...register(`${fieldArrayName}.state`, { required: 'State is required' })} required>
              ...
              <Field label="Zip" error={errors?.[fieldArrayName]?.zip} className="w-1/3">
                <Input
                  {...register(`${fieldArrayName}.zip` as const, { required: 'Zip is required' })}
                  id="state"
                  type="text"
                  required
                />
          ...
          <div className="form-row">
            <Button type="button" onClick={() => setForm({form: 'primaryContact'})} className="btn btn-thin btn-link"><span className="leftArrow"></span>Prev</Button>
